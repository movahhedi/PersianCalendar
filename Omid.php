...
class persian_calendar
{
			...
			if($format[$i]=='%'){
				...
				switch($format[$i]){
					...
					case 'h':
						...
						break;
					case 'c':
						$output_str.=strftime('%y/%m/%d %I:%M:%S', $timestamp);
						break;
					case 'C':
					...
					case 'w':
						...
						break;
					case 'x':
						$output_str.=strftime('%y/%m/%d', $timestamp);
						break;
					case 'X':
						$output_str.=strftime('%I:%M:%S', $timestamp);
						break;
					case 'g':
					case 'y':
	...
	public function date_g2p($date, $separator = '/', $output_format = 'd/m/Y', $persian = false)
	{
		...
		$date = self::persian_date_utf($output_format, $date, $persian);
		return $date;
	}
	...
	{
		$date = explode($separator, $date);
		$date = self::persian_mktime(0, 0, 0, $date[1], $date[0], $date[2]);
